...
class Game:
    """Coordinates the overall gameplay
    between the player and PC opponent."""

    def __init__(self, grid_size, player_name):
        """Set up the game with grids for both
        player and PC based on grid size."""
    ...
    def _place_ships(self, grid):
        """ Randomly place ships on the grid. For
        simplicity, each ship is 3 cells long."""
        ship_lengths = [3, 2]
        for length in ship_lengths:
            while True:
                ...
                if orientation == 'H':
                    ...
                    ship = [(start_x + i, start_y) for i in range(length)]
                try:
                    grid.add_ship(ship)
                    break
                except ValueError:
